...
# docstyle-ignore
"""
Train the ORACLE chatbot's LLM backend using GOLD (Generalized Online Learning
via Distillation).  A smaller student model is fine-tuned to mimic the output
distribution of a larger teacher model on a conversational dataset, yielding a
compact model suitable for use as the ORACLE chatbot's LLM backend.
...
python scripts/gold.py \\
    ...
    --output_dir oracle-chatbot-model \\
    --num_train_epochs 1 \\
    --push_to_hub
